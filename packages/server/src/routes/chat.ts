...
type DumEUIMessage = UIMessage<ChatMessageMetadata, never, InferUITools<ToolContracts>>;

const submitSchema = z.object({
  id: z.string(),
  messages: z
    .array(
      z.custom<DumEUIMessage>((value) => {
...
function hasPendingToolCalls(message: DumEUIMessage) {
  ...
  .post(
    ...
    async (c) => {
      ...
      const previousMessages = Array.isArray(session.messages)
        ? (session.messages as unknown as DumEUIMessage[])
        : [];
      ...
      const nextMessages = await validateUIMessages<DumEUIMessage>({
      ...
      return result.toUIMessageStreamResponse<DumEUIMessage>({
